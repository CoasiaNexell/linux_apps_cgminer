...
static bool set_work(struct btc08_chain *btc08, struct work *work)
{
	...
	if (btc08->work[btc08->last_queued_id] != NULL) {
		if (opt_debug) {
			char s[512];
			snprintf(s, sizeof(s), "[DELETE OLD WORK] btc08->work[%d] local_job_id %d for job_id %s",
			...
			for (int i=1; i<=btc08->num_active_chips; i++)
			{
				...
				if (0 != gn_irq)
				{
					...
					if (get_nonce(btc08, (uint8_t*)&nonce[0], &chip_id, &micro_job_id))
					{
						...
						if (opt_debug) {
							char s[512];
							snprintf(s, sizeof(s), "[GN] btc08->work[%d] local_job_id:%d for job_id:%s", (job_id - 1),job_id, work->job_id);
							dump_work(s, work);
						}
